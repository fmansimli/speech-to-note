<template>
  <div>
    ...
    <div class="login-wrap">
      <h2>Login</h2>
      <div class="form">
        <input type="text" v-model="log.email" placeholder="Email"/>
        <input type="password" v-model="log.password" placeholder="Password" />
        <button @click="loginTo"> Sign in </button>
        <router-link to="/register"> <p> Don't have an account? Register </p></router-link>
        <br>
        <a style="color:red;"><p>{{$route.hash==="#reject"?'username or password incorrect!!!':''}}</p></a> 
      </div>
      <br>
    </div>
  </div>
